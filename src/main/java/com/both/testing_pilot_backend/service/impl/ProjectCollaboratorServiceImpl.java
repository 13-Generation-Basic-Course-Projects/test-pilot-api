...
public class ProjectCollaboratorServiceImpl implements ProjectCollaboratorService {
    ...
    public void inviteCollaborator(ProjectCollaboratorRequest request) {
        ...
        if (projectCollaboratorRepository.isProjectCollaborator(request.getProjectId(), invitedUser.getUserId())) {
            throw new DuplicateKeyException("User '" + request.getCollaboratorEmail() + "' is already a verified collaborator on this project.");
        }
        if (projectCollaboratorRepository.isUnverifiedCollaborator(request.getProjectId(), invitedUser.getUserId())) {
            throw new DuplicateKeyException("User '" + request.getCollaboratorEmail() + "' has already been invited and is awaiting verification for this project. Please wait or contact the project owner.");
        }
        ...
        try {
            ...
            throw new DuplicateKeyException("Failed to invite collaborator: User '" + request.getCollaboratorEmail() + "' is already a collaborator on this project (DB conflict).", e);
        }

        String verificationLink = String.format("%s/api/v1/collaborators/verify?token=%s", appBaseUrl, verificationToken);
